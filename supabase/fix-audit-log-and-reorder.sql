-- Fix audit_log table by adding missing column
...
-- Fix duplicate sibling_order values for existing data
-- This will renumber all children of each parent to have unique sequential orders
...
WHERE p.id = pc.id
  AND p.sibling_order != pc.new_order;
