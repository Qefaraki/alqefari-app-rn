-- Migration: Fix Batch Save Actor ID Foreign Key (Bug #12)
-- Issue: audit_log_enhanced.actor_id FK requires auth.users.id, but function uses profiles.id
-- Error: "violates foreign key constraint audit_log_enhanced_actor_id_fkey"
-- Root Cause: Variable confusion - v_actor_id contains profiles.id, used for both FKs
-- Fix: Separate v_auth_user_id (auth.users.id) from v_actor_profile_id (profiles.id)
-- Pattern: Matches undo_profile_update and other working functions
-- Status: BUG #12 - Discovered during sixth test attempt
...
AS $$
DECLARE
  -- BUG #12 FIX: Separate auth.users.id from profiles.id
  v_auth_user_id uuid;              -- For audit_log_enhanced.actor_id (FK to auth.users.id)
  v_actor_profile_id uuid;          -- For permissions and operation_groups.created_by (FK to profiles.id)
  v_actor_role text;
...
BEGIN
  ...
  -- BUG #12 FIX: Get auth.users.id first
  ...
  -- Get profile ID for permission checks and operation_groups FK
  ...
  -- Use v_actor_profile_id for permission checks (profiles.id)
  ...
  -- Use v_actor_profile_id for operation_groups.created_by FK (profiles.id)
  ...
  LOOP
    ...
    INSERT INTO profiles (name, gender, generation, father_id, mother_id, kunya, nickname, status, sibling_order, dob_data, dod_data, bio, birth_place, current_residence, occupation, education, phone, email, photo_url, social_media_links, achievements, timeline, dob_is_public, profile_visibility, version)
    VALUES (v_child->>'name', v_child->>'gender', v_calculated_generation, CASE WHEN p_parent_gender = 'male' THEN p_parent_id ELSE p_selected_father_id END, CASE WHEN p_parent_gender = 'female' THEN p_parent_id ELSE p_selected_mother_id END, v_child->>'kunya', v_child->>'nickname', COALESCE(v_child->>'status', 'alive'), COALESCE((v_child->>'sibling_order')::integer, 0), (v_child->'dob_data')::jsonb, (v_child->'dod_data')::jsonb, v_child->>'bio', v_child->>'birth_place', v_child->>'current_residence', v_child->>'occupation', v_child->>'education', v_child->>'phone', v_child->>'email', v_child->>'photo_url', (v_child->'social_media_links')::jsonb, CASE WHEN v_child ? 'achievements' AND jsonb_typeof(v_child->'achievements') = 'array' THEN ARRAY(SELECT jsonb_array_elements_text(v_child->'achievements')) ELSE NULL END, (v_child->'timeline')::jsonb, COALESCE((v_child->>'dob_is_public')::boolean, false), COALESCE(v_child->>'profile_visibility', 'family'), 1)
    RETURNING id INTO v_new_profile_id;

    -- BUG #12 FIX: Use v_auth_user_id for audit_log_enhanced.actor_id (auth.users.id)
  ...
  RETURN jsonb_build_object('success', true, 'operation_group_id', v_operation_group_id, 'created_count', v_created_count, 'updated_count', v_updated_count, 'deleted_count', v_deleted_count, 'message', format('تمت معالجة %s عملية بنجاح (إنشاء: %s، تحديث: %s، حذف: %s)', v_batch_size, v_created_count, v_updated_count, v_deleted_count));
...
COMMENT ON FUNCTION admin_quick_add_batch_save IS 'Atomic batch operation for QuickAdd overlay. Fixed Bug #12: Uses auth.uid() for audit_log actor_id FK (auth.users.id), profiles.id for operation_groups.created_by FK (profiles.id). All 12 bugs resolved.';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '=================================================================';
  RAISE NOTICE 'MIGRATION 20251015270000: Fix Actor ID FK (Bug #12)';
  RAISE NOTICE '=================================================================';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed Bug #12: Foreign key constraint violation';
  RAISE NOTICE '';
  RAISE NOTICE '❌ BROKEN: audit_log_enhanced.actor_id = profiles.id';
  RAISE NOTICE '✅ FIXED: audit_log_enhanced.actor_id = auth.uid() (auth.users.id)';
  RAISE NOTICE '';
  RAISE NOTICE '📋 Variable separation:';
  RAISE NOTICE '   ✅ v_auth_user_id = auth.uid() → audit_log_enhanced.actor_id';
  RAISE NOTICE '   ✅ v_actor_profile_id = profiles.id → operation_groups.created_by + permissions';
  RAISE NOTICE '';
  RAISE NOTICE '📋 Changes in 3 locations:';
  RAISE NOTICE '   ✅ CREATE loop - audit log uses v_auth_user_id';
  RAISE NOTICE '   ✅ UPDATE loop - audit log uses v_auth_user_id';
  RAISE NOTICE '   ✅ DELETE loop - audit log uses v_auth_user_id';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 Matches pattern from undo_profile_update and other working functions';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 All 12 bugs now fixed:';
  RAISE NOTICE '   ✅ Bug #1: COALESCE anti-pattern';
  RAISE NOTICE '   ✅ Bug #2: Audit log timing';
  RAISE NOTICE '   ✅ Bug #3: auth_user_id column';
  RAISE NOTICE '   ✅ Bug #4: permission_level column';
  RAISE NOTICE '   ✅ Bug #5: Console logging spam';
  RAISE NOTICE '   ✅ Bug #6: Custom error codes';
  RAISE NOTICE '   ✅ Bug #7: operation_groups NOT NULL';
  RAISE NOTICE '   ✅ Bug #8: group_type CHECK constraint';
  RAISE NOTICE '   ✅ Bug #9: Schema mismatch';
  RAISE NOTICE '   ✅ Bug #10: Achievements array casting';
  RAISE NOTICE '   ✅ Bug #11: Audit log column name';
  RAISE NOTICE '   ✅ Bug #12: Actor ID foreign key (this migration)';
  RAISE NOTICE '';
  RAISE NOTICE '🚀 Function should NOW FINALLY work!';
  RAISE NOTICE '🎯 Ready for seventh test attempt';
  RAISE NOTICE '';
  RAISE NOTICE '=================================================================';
END $$;
