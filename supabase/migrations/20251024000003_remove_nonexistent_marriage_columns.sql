-- Remove references to non-existent marriage columns
--
-- ERROR: column "spouse_father_id" does not exist
-- These columns were copied from an old archived migration but never actually existed
-- in the marriages table schema. Removing them fixes code error 42703.
--
-- Current Marriages Table Schema:
-- - id, husband_id, wife_id, munasib
-- - start_date, end_date, status
-- - created_at, updated_at, deleted_at
-- - created_by, updated_by, marriage_order
-- - start_date_backup, end_date_backup, is_current
--
-- The "with_spouse_parents" stat was never displayed in the UI anyway.
-- The actual business metric exists in munasib_stats CTE using profiles.father_id/mother_id
...
MIGRATION HISTORY:
- 20251023000000: Initial function (had non-existent columns from archived migration)
- 20251024000000: Fixed permission check (still had schema mismatches)
- 20251024000001: Fixed user_id column reference (still had marriage columns)
- 20251024000003: FINAL FIX - Removed non-existent spouse_father_id/spouse_mother_id columns

The "with_spouse_parents" stat was never displayed in UI and was attempting to reference
columns that never existed in the marriages table. The actual metric (munasib with parents)
is correctly calculated in munasib_stats CTE using profiles.father_id/mother_id.

Modified: 2025-10-24 - Final schema fix (removed marriage columns 42703 error)';
