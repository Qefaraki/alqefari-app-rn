-- Migration 087: Add family_origin to admin_update_profile whitelist
-- Bug fix: Munasib name edits don't update family_origin field
-- Issue: When editing wife name in Family Tab, family_origin stays NULL
-- Created: 2025-01-13
...
BEGIN
    ...
    UPDATE profiles SET
        ...
        -- ⭐ NEW: Add family_origin to whitelist (THE FIX!)
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 087 complete: family_origin added to admin_update_profile whitelist';
  RAISE NOTICE '';
  RAISE NOTICE 'Fixed: Munasib name editing in Family Tab';
  RAISE NOTICE '  - Spouse name changes will now update family_origin automatically';
  RAISE NOTICE '  - Handles Al-Qefari detection (cousin marriages set to NULL)';
  RAISE NOTICE '  - Validates minimum 2 words (name + surname)';
  RAISE NOTICE '';
  RAISE NOTICE 'Usage: admin_update_profile now accepts family_origin in p_updates JSONB';
END $$;
