-- Migration: Fix "cannot extract elements from a scalar" error in undo_profile_update
-- Created: 2025-10-15
-- Description: Adds type checking before extracting achievements array from JSONB
--
-- PROBLEM:
-- When old_data contains "achievements": null (JSONB null, not missing key),
-- the function tries to extract array elements from a scalar null value, causing:
-- ERROR: "cannot extract elements from a scalar"
--
-- ROOT CAUSE:
-- Line 169: achievements = CASE WHEN v_old_data ? 'achievements'
--           THEN ARRAY(SELECT jsonb_array_elements_text(v_old_data->'achievements'))...
-- The '?' operator only checks if key exists, not if value is an array.
--
-- SOLUTION:
-- Add jsonb_typeof() check to ensure value is actually an array before extraction.
-- This pattern is already proven in batch_save functions.
--
-- AFFECTED LINE:
-- Before: CASE WHEN v_old_data ? 'achievements' THEN...
-- After:  CASE WHEN v_old_data ? 'achievements' AND jsonb_typeof(v_old_data->'achievements') = 'array' THEN...
...
FIXES APPLIED:
1. SCALAR ERROR FIX (v2025-10-15-02):
...
2. FOREIGN KEY FIX (v2025-10-15-01):
...
- achievements: Only restored if valid array type (NEW)
