-- Safe migration for profile photos gallery (no admin column required)
...
-- Create RLS policies (allow authenticated users to manage photos)
...
-- Create or replace function to add profile photo (no admin check)
...
-- Create or replace function to set primary photo (no admin check)
...
-- Create or replace function to delete profile photo (no admin check)
...
-- Create or replace function to reorder photos (no admin check)
...
-- Migrate existing photos to the new table (if any exist in profiles table)
-- This will create an entry for profiles that already have a photo_url
