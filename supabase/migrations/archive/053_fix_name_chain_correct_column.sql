-- Fix profile matching with correct column names and proper chain building
...
BEGIN
  ...
  SELECT 
    ...
    p.generation,
    p.user_id IS NOT NULL as has_auth,  -- Changed from auth_user_id to user_id
  ...
  ORDER BY 
    ...
    CASE 
      ...
      ELSE 20
    END DESC,
    p.user_id IS NULL DESC,  -- Changed from auth_user_id to user_id
    p.generation DESC,
    p.name ASC
  LIMIT 20;
END;
