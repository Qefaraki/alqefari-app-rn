-- Migration: Standardize Core Statistics Timeout
-- Date: October 28, 2025
-- Purpose: Change core stats timeout from 2s to 3s to match system-wide pattern
--   - Permission checks use 3s
--   - Phone change uses 3s
--   - Deep link checks use 3s
--   - Core stats should match for consistency
...
DECLARE
  ...
  user_role text;
BEGIN
  -- Match system-wide timeout pattern (permission checks, phone change, etc.)
  SET LOCAL statement_timeout = '3000';  -- Changed from 2000
...
-- Comment
COMMENT ON FUNCTION admin_get_core_statistics() IS 'Returns core family statistics (gender, generations, vital status, data quality) with 3-second timeout matching system-wide pattern. Admin-only.';
