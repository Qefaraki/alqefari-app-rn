-- Migration: Fix admin_update_profile_crop to use audit_log_enhanced with JSONB
-- Author: Claude Code
-- Date: 2025-10-27
-- Purpose: Fix activity log integration to use correct table and JSONB format
-- Issue: Original RPC tried to INSERT INTO activity_log (doesn't exist)
-- Solution: Use audit_log_enhanced with JSONB old_data/new_data format

-- ============================================================================
-- FUNCTION: admin_update_profile_crop (Fixed Activity Log)
-- ============================================================================
...
BEGIN
  ...
  UPDATE profiles SET
    ...
    version = version + 1,
    updated_at = NOW(),
    updated_by = auth.uid()  -- Use auth.uid() for consistency
  ...
  -- ========================================
  -- ACTIVITY LOG: Undo System Integration (FIXED)
  -- ========================================

  -- Generate or use provided operation group ID
  v_group_id := COALESCE(p_operation_group_id, gen_random_uuid());

  -- ✅ FIX: Use audit_log_enhanced with JSONB format
  -- Insert single entry with all 4 crop fields in old_data/new_data
  INSERT INTO audit_log_enhanced (
    table_name,
    record_id,
    action_type,
    actor_id,  -- auth.users.id, not profiles.id
    old_data,  -- JSONB with all old crop values
    new_data,  -- JSONB with all new crop values
    ...
    p_profile_id,
    'crop_update',
    auth.uid(),  -- ✅ Use auth.uid() not p_user_id
    jsonb_build_object(
...
COMMENT ON FUNCTION admin_update_profile_crop IS
  'Atomically update all 4 crop fields with comprehensive validation and FIXED activity log integration.

ACTIVITY LOG FIX (2025-10-27):
- Uses audit_log_enhanced table (not activity_log which doesn''t exist)
- JSONB format for old_data/new_data (not TEXT old_value/new_value)
- Single entry with all 4 fields (not 4 separate entries)
- changed_fields as ARRAY (not field_name as TEXT)
- actor_id uses auth.uid() (auth.users.id) for FK constraint
- Supports undo system via operation_group_id
...
--   p_profile_id := 'abc-123'::UUID,
--   p_crop_top := 0.25,
--   p_crop_bottom := 0.10,
--   p_crop_left := 0.05,
--   p_crop_right := 0.05,
--   p_version := 5,
--   p_user_id := 'user-456'::UUID
-- );
