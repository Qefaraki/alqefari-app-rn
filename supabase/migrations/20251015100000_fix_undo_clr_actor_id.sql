-- Migration: Fix CLR actor_id foreign key in undo_profile_update
-- Created: 2025-10-15
-- Description: Changes CLR insert to use auth.uid() for actor_id instead of profiles.id
--
-- PROBLEM:
-- The undo function uses v_current_user_id (profiles.id) for CLR actor_id,
...
--
-- SOLUTION:
-- Use auth.uid() for CLR actor_id (consistent with updated_by fix from migration 090000)
...
- updated_by uses auth.uid() (auth.users.id) instead of profiles.id
- actor_id in CLR also uses auth.uid() for consistency
- This matches the profiles_updated_by_fkey and audit_log_enhanced_actor_id_fkey constraints
