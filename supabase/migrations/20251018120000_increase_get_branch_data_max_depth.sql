-- Increase get_branch_data max_depth limit from 10 to 15
--
-- Root cause: Frontend changed to request depth=15 for future-proofing deeper tree structures
-- Current database constraint: max_depth must be between 1 and 10
-- Error: "max_depth must be between 1 and 10" when calling get_branch_data(hid, 15, 5000)
--
-- Solution: Update validation constraint to allow max_depth up to 15
--
-- Impact: No performance impact (depth is traversal limit, not data volume)
-- Current max tree depth: 7 levels (well within new limit of 15)
-- Change: Line 66 - Update validation from 10 to 15
...
AS $function$
BEGIN
    -- Input validation (UPDATED: max_depth 10 → 15)
...
-- Expected: All profiles loaded without "max_depth must be between 1 and 10" error
