-- Migration: Fix undo updated_by foreign key constraint
-- Created: 2025-10-15
-- Description: Fixes foreign key violation in undo_profile_update by using
--              auth.uid() instead of profiles.id for updated_by column
--
-- PROBLEM:
-- The undo function was setting updated_by to v_current_user_id which comes from:
--   SELECT id FROM profiles WHERE user_id = auth.uid()
-- This returns profiles.id, but updated_by foreign key references auth.users.id
--
-- Error: "insert or update on table profiles violates foreign key constraint profiles_updated_by_fkey"
--
-- SOLUTION:
-- Use auth.uid() directly for updated_by (returns auth.users.id)
-- This matches the foreign key constraint and is consistent with admin_update_profile()
--
-- FOREIGN KEY DEFINITION:
-- profiles.updated_by → auth.users.id
...
BEGIN
  ...
  SET
    ...
    -- Metadata (CRITICAL FIX: Use auth.uid() for updated_by)
    version = version + 1,
    updated_at = NOW(),
    updated_by = auth.uid()  -- FIX: Use auth.users.id, not profiles.id
...
'Safely undoes a profile update with proper foreign key handling.

FOREIGN KEY FIX (v2025-10-15):
- updated_by now uses auth.uid() (auth.users.id) instead of profiles.id
- This matches the profiles_updated_by_fkey foreign key constraint
- Consistent with admin_update_profile() and other admin functions
