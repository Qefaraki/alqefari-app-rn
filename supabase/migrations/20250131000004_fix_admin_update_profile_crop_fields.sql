/**
 * Migration: Fix admin_update_profile RPC - Remove Crop Fields
 * Date: 2025-01-31
 *
 * Purpose: Remove crop field references from admin_update_profile RPC that
 *          were missed when Migration 002 didn't fully execute.
 *
 * Issue: ProfileViewer save failing with "column crop_metadata does not exist"
 *        because admin_update_profile still had crop field assignments.
 *
 * Removed fields from UPDATE statement:
 * - crop_metadata (JSONB)
 * - crop_top (NUMERIC)
 * - crop_bottom (NUMERIC)
 * - crop_left (NUMERIC)
 ...
 *
 * Added field:
 * - photo_url_cropped (TEXT) - file-based cropped photo URL
 *
 * Impact: Profile editing now works correctly after crop column removal.
 */
...
BEGIN
    ...
    -- Update profile with whitelisted fields (crop fields REMOVED)
    UPDATE profiles SET
        ...
        photo_url_cropped = CASE WHEN p_updates ? 'photo_url_cropped' THEN (p_updates->>'photo_url_cropped')::TEXT ELSE photo_url_cropped END,
...
COMMENT ON FUNCTION admin_update_profile IS
  'Update profile with permission checking and optimistic locking.
...
-- ============================================================================
-- Fixed: admin_update_profile now works without crop field errors
-- Verified: Profile editing functionality restored
