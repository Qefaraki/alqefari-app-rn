-- Migration: Fix Batch Save group_type CHECK Constraint Violation
-- Issue: group_type column has CHECK constraint allowing only specific values
-- Allowed: 'cascade_delete', 'batch_update', 'merge_profiles', 'relationship_change'
-- Error: "new row violates check constraint operation_groups_group_type_check"
-- Fix: Change 'quick_add_batch' to 'batch_update' (semantically correct)
-- Status: BUG #8 - Discovered during second test attempt
...
BEGIN
  ...
  -- FIX: Use 'batch_update' instead of 'quick_add_batch' (allowed by CHECK constraint)
...
'Atomic batch operation for QuickAdd overlay. Fixed: group_type uses batch_update (allowed by CHECK constraint). All bugs resolved.';

DO $$
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '=====================================================';
  RAISE NOTICE 'MIGRATION 20251015230000: Fix group_type CHECK';
  RAISE NOTICE '=====================================================';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed Bug #8: group_type CHECK constraint';
  RAISE NOTICE '✅ Changed quick_add_batch → batch_update';
  RAISE NOTICE '✅ Allowed values: cascade_delete, batch_update, merge_profiles, relationship_change';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 Function should NOW work (8 bugs fixed!)';
  RAISE NOTICE '';
  RAISE NOTICE '=====================================================';
END $$;
