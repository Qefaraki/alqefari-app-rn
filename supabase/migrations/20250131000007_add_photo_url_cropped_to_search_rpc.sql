/**
 * Migration: Add photo_url_cropped to search_name_chain RPC
 * Date: 2025-01-31
 * Migration Number: 007
 *
 * Purpose: Add photo_url_cropped field to search RPC for cropped photo display in search results.
 *
 * Background:
 * - Search results currently show original uncropped photos
 * - This causes inconsistent UX between search and profile viewer
 * - Migration 003 removed old crop fields, this adds new file-based field
 *
 * Changes:
 * - Dropped old function version (cannot change return type without DROP)
 * - Added photo_url_cropped to RETURNS TABLE (after photo_url)
 * - Added p.photo_url_cropped to ancestry CTE (base + recursive)
 * - Added photo_url_cropped to matches CTE
 * - Added photo_url_cropped to marriage_check CTE
 * - Added photo_url_cropped to final SELECT
 *
 * Impact: Search results will now display cropped photos when available.
 */
...
BEGIN
  ...
  WITH RECURSIVE ancestry AS (
    SELECT
      ...
      1 as depth, p.generation, p.photo_url, p.photo_url_cropped,  -- ADDED
    ...
    SELECT
      ...
      a.depth + 1, a.generation, a.photo_url, a.photo_url_cropped,  -- ADDED
  ...
  matches AS (
    SELECT DISTINCT ON (a.id)
      ...
      a.generation, a.photo_url, a.photo_url_cropped,  -- ADDED
      a.birth_year_hijri, a.death_year_hijri,
      CASE
  ...
  SELECT mc.id, mc.hid, mc.name, mc.name_chain, mc.generation, mc.photo_url, mc.photo_url_cropped,  -- ADDED
...
COMMENT ON FUNCTION search_name_chain IS
  'Search profiles by name chain (ancestry path) with fuzzy matching.
   Updated 2025-01-31: Added photo_url_cropped for file-based cropping display in search results.';

-- ============================================================================
-- Migration 007 Complete! ✅
-- ============================================================================
-- Fixed: Search results will now display cropped photos when available
-- Verified: photo_url_cropped field added to all 6 locations in RPC
