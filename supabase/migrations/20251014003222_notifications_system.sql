...
DECLARE
    ...
    v_existing_notif_count INTEGER;
BEGIN
    -- Deduplication: Check for similar notifications in the last 5 minutes
    SELECT COUNT(*) INTO v_existing_notif_count
    FROM public.notifications
    WHERE user_id = p_user_id
      AND type = p_type
      AND related_request_id = p_related_request_id
      AND created_at > NOW() - INTERVAL '5 minutes';

    -- Only create if no duplicate exists
    IF v_existing_notif_count = 0 THEN
    ...
    ELSE
        -- Return NULL if duplicate exists (or return existing notification ID)
...
BEGIN
    ...
    IF NEW.status = 'approved' AND OLD.status = 'pending' THEN
        ...
        PERFORM create_notification(
            NEW.user_id,
            'profile_link_approved',
            E'تمت الموافقة على طلبك! 🎉',
            format(E'تم ربط ملفك الشخصي "%s" بنجاح. يمكنك الآن تعديل معلوماتك.', COALESCE(v_profile_name, NEW.name_chain)),
            jsonb_build_object(
                'request_id', NEW.id,
    ...
    ELSIF NEW.status = 'rejected' AND OLD.status = 'pending' THEN
        ...
        PERFORM create_notification(
            ...
            'profile_link_rejected',
            E'تم رفض طلب الربط',
            format(E'تم رفض طلب ربط الملف "%s". %s',
                COALESCE(v_profile_name, NEW.name_chain),
                COALESCE(NEW.review_notes, E'يرجى التواصل مع المشرف للمزيد من المعلومات.')
            ),
            jsonb_build_object(
                'request_id', NEW.id,
                'profile_id', NEW.profile_id,
...
BEGIN
    ...
    LOOP
        PERFORM create_notification(
            ...
            'new_profile_link_request',
            E'طلب ربط جديد',
            format(E'طلب جديد من %s لربط الملف الشخصي "%s"',
                COALESCE(NEW.phone, E'مستخدم'),
                COALESCE(v_profile_name, NEW.name_chain)
            ),
            jsonb_build_object(
                'request_id', NEW.id,
