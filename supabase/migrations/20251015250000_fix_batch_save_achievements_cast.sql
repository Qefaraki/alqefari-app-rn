-- Migration: Fix Batch Save Achievements Casting Error (Bug #10)
-- Issue: Cannot cast type jsonb to text[] when updating achievements field
-- Error: "فشلت العملية الجماعية: cannot cast type jsonb to text[]"
-- Root Cause: Direct cast (v_child->'achievements')::text[] fails at runtime
-- Fix: Use proper JSONB array to text[] conversion with type checking
-- Status: BUG #10 - Discovered during fourth test attempt
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO profiles (
      ...
      -- Bug #10 Fix: Proper JSONB array to text[] conversion for CREATE
      CASE
  ...
  LOOP
    ...
    -- BUG #10 FIX: Proper JSONB to text[] conversion for achievements
    ...
    SET
      ...
      -- BUG #10 FIX: Proper JSONB array to text[] conversion
      -- Cannot use simple cast: (v_child->'achievements')::text[]
      -- Must use explicit conversion with type check
      achievements = CASE
        WHEN v_child ? 'achievements' AND jsonb_typeof(v_child->'achievements') = 'array' THEN
          ARRAY(SELECT jsonb_array_elements_text(v_child->'achievements'))
        ELSE achievements
...
'Atomic batch operation for QuickAdd overlay. Fixed Bug #10: Proper JSONB to text[] conversion for achievements (jsonb_array_elements_text). All 10 bugs resolved. Complete 20-field support.';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '=================================================================';
  RAISE NOTICE 'MIGRATION 20251015250000: Fix Achievements Casting (Bug #10)';
  RAISE NOTICE '=================================================================';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Fixed Bug #10: JSONB to text[] casting error';
  RAISE NOTICE '';
  RAISE NOTICE '❌ BROKEN: (v_child->''achievements'')::text[]';
  RAISE NOTICE '✅ FIXED: ARRAY(SELECT jsonb_array_elements_text(v_child->''achievements''))';
  RAISE NOTICE '';
  RAISE NOTICE '📋 Changes:';
  RAISE NOTICE '   ✅ Added type check: jsonb_typeof() = ''array''';
  RAISE NOTICE '   ✅ Explicit conversion: jsonb_array_elements_text()';
  RAISE NOTICE '   ✅ Applied to both CREATE and UPDATE loops';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 All 10 bugs now fixed:';
  ...
  RAISE NOTICE '   ✅ Bug #10: Achievements array casting (this migration)';
  RAISE NOTICE '';
  RAISE NOTICE '🚀 Function should NOW work (proper array conversion!)';
  RAISE NOTICE '🎯 Ready for fifth test attempt';
  RAISE NOTICE '';
  RAISE NOTICE '=================================================================';
END $$;
