-- Ensure onboarding search never returns Munasib profiles by requiring HID
...
BEGIN
  ...
  WITH RECURSIVE name_chain AS (
    -- Start with each matching profile in the core tree (has HID)
    SELECT
      p.id as profile_id,
      p.id as current_id,
    ...
    WHERE p.deleted_at IS NULL
      AND p.user_id IS NULL  -- Only search unlinked profiles
