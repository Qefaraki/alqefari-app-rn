...
const ModernProfileEditorV4 = ({ visible, profile, onClose, onSave }) => {
  ...
  // Render fixed header using Expo UI native components
  const renderFixedHeader = () => {
    if (Platform.OS === 'ios') {
            ...
            <HStack spacing={16}>
              ...
              >
                {saving ? (
                  ...
                  <UIText
                    color={hasChanges ? "#007AFF" : "#007AFF"}
                    size={17}
                    weight="semibold"
      ...
      // Android fallback using Jetpack Compose
      return (
        <SafeAreaView style={styles.fixedHeaderContainer} edges={['top']}>
  ...
  return (
    <>
      {/* Fixed Header - Outside BottomSheet */}
      {renderFixedHeader()}
...
const styles = StyleSheet.create({
  // Fixed header styles (using Expo UI native components)
  ...
  scrollContent: {
    paddingTop: 110, // Account for fixed header (SafeAreaView + 60px header) + spacing
