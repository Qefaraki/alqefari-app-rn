...
// Version WITHOUT useMemo - always recalculates
export function useFormattedDateNoMemo(
  ...
  // NO MEMOIZATION - calculate on every render
  if (!date) return '';
...
// Convenience hooks
export function useRelativeDateNoMemo(date: Date | string | number | null | undefined): string {
  return useFormattedDateNoMemo(date, { relative: true });
}

export function useAbsoluteDateNoMemo(date: Date | string | number | null | undefined): string {
  return useFormattedDateNoMemo(date, { relative: false });
}