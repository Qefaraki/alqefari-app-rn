...
async function executeSQL() {
  console.log('Deploying complete audit system fix...\n');
  ...
  const options = {
    ...
    headers: {
      ...
      'Content-Length': Buffer.byteLength(JSON.stringify({ p_sql: sql })),
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200 || res.statusCode === 201 || res.statusCode === 204) {
          console.log('✅ Complete audit system fix deployed!');
          console.log('- Created audit_log_enhanced table');
          console.log('- Updated trigger to use new table');
          console.log('- Simplified admin_create_munasib_profile');
          console.log('\nSpouse creation should work now!');
          resolve();
        } else {
    ...
    req.write(JSON.stringify({ p_sql: sql }));
    req.end();
  });
}
