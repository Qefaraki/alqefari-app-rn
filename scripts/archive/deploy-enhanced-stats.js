...
-- Create the new enhanced statistics function
...
DECLARE
    result json;
BEGIN
    -- Skip auth check for deployment
    WITH stats AS (
        SELECT 
            -- Basic counts
    ...
    munasib_families AS (
        ...
        SELECT 
            COALESCE(
                CASE 
                    WHEN p.father_id IS NOT NULL THEN 
                        (SELECT display_name FROM profiles WHERE id = p.father_id)
                    ELSE p.display_name 
                END,
                'غير محدد'
            ) as family_name,
...
async function deployEnhancedStats() {
  console.log("Deploying enhanced statistics function...\n");
  ...
  try {
    ...
    // Try to execute SQL via admin_execute_sql RPC
    const { error: execError } = await supabase.rpc("admin_execute_sql", {
      sql: enhancedStatsSQL,
    });

    if (execError) {
      console.error("Could not deploy via RPC:", execError);

      // Try direct query as fallback
      console.log("Trying direct deployment...");

      // This would need service role key, which we don't have in frontend
      console.log(
        "\nSQL has been prepared. Please run this in Supabase SQL editor:",
      );
      console.log("----------------------------------------");
      console.log(enhancedStatsSQL);
      console.log("----------------------------------------");
    } else {
      console.log("✓ Enhanced statistics function deployed successfully!");
    }
    ...
    if (!error && data?.basic) {
      console.log("✓ Function is working with new structure!");
      console.log("\nStats summary:");
      console.log(`- Total profiles: ${data.basic.total_profiles}`);
      console.log(`- Munasib count: ${data.munasib.total_munasib}`);
      console.log(
        `- Top families: ${data.munasib.top_families?.length || 0} families`,
      );
    } else {
      console.log("Function returns old structure, needs manual deployment");
    }
  } catch (err) {
