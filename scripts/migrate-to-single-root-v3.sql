...
BEGIN
    ...
    -- Get root info
    SELECT name, descendants_count INTO root_name, root_descendants
    FROM profiles 
    WHERE father_id IS NULL 
    AND mother_id IS NULL 
    AND deleted_at IS NULL
    ...
    RAISE NOTICE '  Root descendants: %', root_descendants;
...
SELECT 'New tree structure sample:' as info;
SELECT hid, name, generation, 
       CASE WHEN father_id IS NULL THEN 'ROOT' ELSE 'Has Father' END as parent_status
FROM profiles 
WHERE generation <= 2
AND deleted_at IS NULL
