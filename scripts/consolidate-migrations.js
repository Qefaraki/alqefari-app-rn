...
// Migrations to keep (recent and important ones)
const KEEP_MIGRATIONS = [
  '20250927114554_profile_link_improvements_fixed.sql',
  '20250927134328_profile_link_improvements.sql',
  '20250927134329_critical_functions_only.sql',
  '057_create_activity_log_view.sql',
  '057_fix_audit_trigger_use_enhanced.sql',
  '058_fix_marriage_functions_audit.sql',
];

// Get all migration files
const allFiles = fs.readdirSync(migrationsDir);
const migrations = allFiles.filter(file =>
  file.endsWith('.sql') &&
  !KEEP_MIGRATIONS.includes(file) &&
  file !== 'consolidated_baseline.sql'
);

console.log(`Found ${migrations.length} migrations to archive`);
console.log(`Keeping ${KEEP_MIGRATIONS.length} recent migrations`);
...
console.log(`\nArchived ${archivedCount} migration files`);

// Create consolidated baseline migration
console.log('\nCreating consolidated baseline migration...');

const consolidatedSQL = `-- Consolidated Baseline Migration
...
const consolidatedPath = path.join(migrationsDir, '000_consolidated_baseline.sql');
fs.writeFileSync(consolidatedPath, consolidatedSQL);

console.log('✓ Created consolidated baseline migration: 000_consolidated_baseline.sql');

// Create summary report
const report = {
  timestamp: new Date().toISOString(),
  archivedMigrations: archivedCount,
  keptMigrations: KEEP_MIGRATIONS.length,
  consolidatedFile: '000_consolidated_baseline.sql',
  archiveDirectory: 'supabase/migrations/archive/',
};

const reportPath = path.join(migrationsDir, 'consolidation-report.json');
fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));

console.log('\n📊 Consolidation Summary:');
console.log(`  - Archived: ${archivedCount} files`);
console.log(`  - Kept: ${KEEP_MIGRATIONS.length} recent migrations`);
console.log(`  - Created: 1 consolidated baseline`);
